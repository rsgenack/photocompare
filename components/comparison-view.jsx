...
export default function ComparisonView({
  ...
  // Add logging for image clicks
  const handleLeftImageClick = (e) => {
    e.preventDefault()
    e.stopPropagation()
    logger.info('Left image clicked', { imageId: leftImage?.id })
    onSelectLeft()
  }
  ...
  const handleRightImageClick = (e) => {
    e.preventDefault()
    e.stopPropagation()
    logger.info('Right image clicked', { imageId: rightImage?.id })
    onSelectRight()
  }

  // Add logging for image removal
  ...
  // Add logging for zoom changes
  const handleZoomChange = (value) => {
    logger.debug('Zoom level changed', { from: zoom, to: value })
    setZoom(value)
  }
  ...
  return (
    <div className="p-6 space-y-6" style={{ WebkitTapHighlightColor: "transparent" }}>
      ...
      {/* Instructions with the same colors as the previous buttons */}
      <div className="flex flex-col md:flex-row gap-4 mb-4">
        <div className="flex-1 py-3 text-center font-bold bg-cardinal text-white border-2 border-black">
          CLICK LEFT IMAGE OR PRESS <ArrowLeft className="h-5 w-5 mx-1 inline" /> TO SELECT
        </div>
        <div className="flex-1 py-3 text-center font-bold bg-selective_yellow text-black border-2 border-black">
          CLICK RIGHT IMAGE OR PRESS <ArrowRight className="h-5 w-5 mx-1 inline" /> TO SELECT
        </div>
      </div>
